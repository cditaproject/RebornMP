...
**RebornMP** — это мультиплеерный фреймворк для Grand Theft Auto V, созданный как современная альтернатива FiveM, RageMP и alt:V. Проект строится на производительном ядре **Rust** (в хуках и памяти игры) и гибкой серверной логике на **TypeScript** (через Bun).

## 🧬 Архитектура проекта

| Компонент | Язык | Тип сборки | Что делает |
|-----------|------|------------|-------------|
| **`client/`** | Rust | `cdylib` (DLL) | Инжектируется в игру. Перехватывает функции (хуки DirectX, сеть), читает/пишет память, отрисовывает UI через WebView2. |
| **`server/`** | Rust (ядро) + TypeScript (скрипты) | `cdylib` + Bun | Rust управляет сокетами и памятью, а скрипты на TS реализуют игровую логику (события, API машин/игроков). |
| **`shared/`** | Rust | `lib` | Общая библиотека с описанием структур пакетов, которые летят между клиентом и сервером. |
| **`sdk/`** | Rust | `lib` | Reversal SDK — «карта памяти игры». Содержит смещения и структуры классов GTA V (CVehicle, CPed и др.). |
| **`launcher/`** | Rust | `bin` (EXE) | Запускает `GTA5.exe` и инжектирует `client.dll` в процесс игры. |
